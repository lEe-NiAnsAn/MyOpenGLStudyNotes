...
static int flashMode = FLASH_ON;		// 默认开启手电筒
...
int main() {
    ...
    // 创建窗口
	GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "Learning Chapter10", NULL, NULL);
    if (window == NULL) {
    ...
    while (!glfwWindowShouldClose(window)){
		...
		float a = 0.9f;  // 短轴半轴长
		float b = 2.0f;  // 长轴半轴长
		glm::vec3 lightPostions[2] = {	// 设置光源位置
			glm::vec3(a * cosf(1.3f * time), 1.0f, b * sinf(1.3f * time) - 1.0f),
			glm::vec3(0.0f, b * sinf(1.3f * time), a * cosf(1.3f * time))
		};
		glm::vec3 lightColors[2] = {glm::vec3(1.0f, 1.0f, 1.0f), glm::vec3(0.0f, 1.0f, 0.0f)};	// 白光 + 绿光
		float linear, quadratic; 	// 光照衰减函数一次项、二次项
        if (lightMode == POINT_LIGHT) {	// 点光源
			linear = 0.22f;
			quadratic = 0.2f;
			for (int i = 0; i < 2; i++) {           
				myShader.setVec3(("pointLights[" + std::to_string(i) + "].position").c_str(), lightPostions[i]);     			// 传入光源位置
				myShader.setVec3(("pointLights[" + std::to_string(i) + "].diffuse").c_str(), glm::vec3(0.8f) * lightColors[i]);	// 传入漫反射光强分量（一白光一绿光）
				myShader.setVec3(("pointLights[" + std::to_string(i) + "].specular").c_str(), lightColors[i]);					// 传入镜面反射光强分量（一白光一绿光）
				myShader.setVec3(("pointLights[" + std::to_string(i) + "].ambient").c_str(), glm::vec3(0.2f) * lightColors[0]);	// 传入环境光强分量（恒白光）
				// 设置光照衰减参数
				myShader.set1Float(("pointLights[" + std::to_string(i) + "].constant").c_str(), 1.0f);
				myShader.set1Float(("pointLights[" + std::to_string(i) + "].linear").c_str(), linear);
				myShader.set1Float(("pointLights[" + std::to_string(i) + "].quadratic").c_str(), quadratic);
			}        
        }
        else if (lightMode == PARALLELT_LIGHT) {	// 平行光源
			direction = glm::vec3(0.6f, -1.0f, 0.0f);									
			myShader.setVec3("paralleltLight.direction", direction);					// 传入光照方向
			myShader.setVec3("paralleltLight.diffuse", glm::vec3(0.8f) * lightColor);	// 传入漫反射光强分量
			myShader.setVec3("paralleltLight.specular", lightColor);					// 传入镜面反射光强分量
			myShader.setVec3("paralleltLight.ambient", glm::vec3(0.2f) * lightColor);	// 传入环境光强分量
		}
        if (!(flashMode == FLASH_OFF)) {	// 聚光源
			direction =myCamera.m_front;                                            
			glm::vec3 lightPos = myCamera.m_position;	                                       
			myShader.setVec3("spotLight.direction", direction);						// 传入光照方向
			myShader.setVec3("spotLight.position", lightPos);	                    // 传入光源位置
			myShader.set1Float("spotLight.innerCutOff", cosf(glm::radians(11.0f)));	// 设置聚光内切光角余弦值
			myShader.set1Float("spotLight.outerCutOff", cosf(glm::radians(13.0f)));	// 设置聚光外切光角余弦值
			myShader.setVec3("spotLight.diffuse", glm::vec3(0.8f) * lightColor);	// 传入漫反射光强分量
			myShader.setVec3("spotLight.specular", lightColor);						// 传入镜面反射光强分量
			myShader.setVec3("spotLight.ambient", glm::vec3(0.2f) * lightColor);	// 传入环境光强分量
			// 设置光照衰减参数
			linear = 0.045f;
			quadratic = 0.075f;
			myShader.set1Float("spotLight.constant", 1.0f);
			myShader.set1Float("spotLight.linear", linear);
			myShader.set1Float("spotLight.quadratic", quadratic);
        }
        myShader.set1Int("lightMode", lightMode);
        myShader.set1Int("flashMode", flashMode);
        ...
        if (lightMode == POINT_LIGHT) {
			for (int i = 0; i < 2; i++) {
				...
				lightShader.setVec3("lightColor", lightColors[i]);	// 一白光一绿光
				glBindVertexArray(VAOLight);
...
void processInput(GLFWwindow *window) {
    ...
    if (currentTState && !lastTState) {
        if (glfwGetKey(window, GLFW_KEY_T) == GLFW_PRESS) { // 按 T 控制手电筒模式
            ...
            std::cout << "The flash mode had been changed." << std::endl;
        }   
    }
