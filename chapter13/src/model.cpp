...
void Model::Draw(Shader shader, const glm::vec3& viewPos) {
    for (unsigned int i = 0; i < opaqueMeshes.size(); i++) {	    // 首先遍历绘制不含透明度物体
        opaqueMeshes[i].Draw(shader);
    }  
    if (!transparentMeshes.empty()) {
        std::multimap<float, Mesh*, std::greater<float>> sortedMap; // 使用 map 降序多重映射比较器通过传入的视角坐标进行手动的深度排序
        for (auto& mesh : transparentMeshes) {
            float distansce = glm::distance(viewPos, mesh.vertices[0].Position);  // 粗略通过各网格的首个顶点坐标计算距离
...
void Model::loadModel(const std::string path) {
    ...
    // 检查场景指针是否为空、场景数据是否完整、是否缺失根节点指针（注意：按位与优先级低于逻辑或）
...
void Model::processNode(aiNode *node, const aiScene *scene) {
    ...
    for(unsigned int i = 0; i < node->mNumMeshes; i++) {
        ...
        float opacity = 1.0f;
        aiGetMaterialFloat(material, AI_MATKEY_OPACITY, &opacity);      // 读取透明度
        if (opacity < 1.0f) {
            transparentMeshes.push_back(processMesh(mesh, scene));
        }
        else {
            opaqueMeshes.push_back(processMesh(mesh, scene));
        }     
