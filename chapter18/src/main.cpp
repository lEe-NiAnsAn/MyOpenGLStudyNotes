...
int main() {
	...
	// 设置窗口选项
	// glfwWindowHint(GLFW_SAMPLES, 4);	// GLFW 自动生成 4× MSAA 抗锯齿帧缓冲
	windowMode = WindowMode::CommonWin;	// 初始为有边框普通窗口，确保存在窗口状态值
    // 创建窗口
	GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "Learning Chapter18", NULL, NULL);
    if (window == NULL) {
	...
	while (!glfwWindowShouldClose(window)){
		...
		if (multiNum != 0) {
			...
			glBlitFramebuffer(0, 0, screenWidth, screenHeight, 0, 0, screenWidth, screenHeight, GL_COLOR_BUFFER_BIT, GL_NEAREST);	// 位块传送缓冲帧数据（源区域左下角和右上角坐标，目标区域左下角和右上角坐标，指定复制操作对象为颜色缓冲区，最近邻插值）	
		}
...
void processInput(GLFWwindow *window) { // 传入窗口句柄，实时监测按键事件
	...
	if (Camera::IsMouseCaptured) {
		...
		// 按 Alt + Enter 切换无边框模式：利用 Windows 的 DWM（桌面窗口管理器）的类三重缓冲架构避免在关闭垂直同步时出现画面撕裂现象（全屏独占模式无法使用 DWM） 
		...
		bool currentAltEnterState = (currentleftAltState || currentrightAltState) && currentEnterState;	// 组合键
		...
		if (currentVState && !lastVState) {
			if (VSync) {
				...
				else if (windowMode == WindowMode::CommonWin) {	// 窗口化模式下必须进入全屏独占模式才可脱离硬件调度强制更改垂直缓冲模式
					switch2FullWin(window);
...
void switch2FramelessWin(GLFWwindow *window){
	...
	glfwSetWindowMonitor(window, NULL, 0, 0, mode->width, mode->height - 1, 0);	// 设置窗口铺满屏幕（窗口高度减一，强制显示任务栏遮盖一部分窗口，防止进入伪全屏状态）
	glfwSetWindowAttrib(window, GLFW_DECORATED, GLFW_FALSE);	// 关闭窗口修饰（移除标题栏、边框等）
...
void setMSAA(FBO& multiple, FBO& intermediate, int multiNum) {
	...
	glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, multiNum, GL_RGB, screenWidth, screenHeight, GL_TRUE);	// （目标类型，样本数，颜色类型，宽，高，对每个纹素使用相同样本位置及子采样点个数）
	glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, 0);
	...
	glGenRenderbuffers(1, &multiple.rbo); 
	glBindRenderbuffer(GL_RENDERBUFFER, multiple.rbo); 
	glRenderbufferStorageMultisample(GL_RENDERBUFFER, multiNum, GL_DEPTH24_STENCIL8, screenWidth, screenHeight);	// (目标类型，样本数，缓冲位数，宽，高)
	glBindRenderbuffer(GL_RENDERBUFFER, 0);
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, multiple.rbo);
	// 检查帧缓冲对象是否完整
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)	
	...
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	// 创建中间帧缓冲对象，解析信息过多的多重采样帧缓冲至普通帧缓冲（多采样自动平均至单一采样）
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glBindTexture(GL_TEXTURE_2D, 0);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, intermediate.textureColorbuffer, 0);	// 仅需颜色纹理无需 RBO
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
